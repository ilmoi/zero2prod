...
impl ResponseError for ValidationError {
    fn status_code(&self) -> StatusCode {
        //todo in the other ones we're happy with a default implementation of 500
        // 1 problem with this approach is that all ValidationErrors used across the app now will have a 400 error code. that may or may not be what we want
...
// impl std::error::Error for ValidationError { //todo can only impl for <dyn Error>
