...
pub async fn subscribe(
    ...
    let mut transaction = pg_pool
        .begin()
        .await //todo now we have to wrap it in box unexpected error and a pointer (Box)
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(PoolError(e))))?;

    // InsertSubscriberError(sqlx::Error),
    let sub_id = insert_subscriber(&mut transaction, &new_subscriber)
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(InsertSubscriberError(e))))?;

    // StoreTokenError(sqlx::Error),
    let sub_token = gen_sub_token();
    store_token(&mut transaction, sub_id, &sub_token)
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(StoreTokenError(e))))?;

    // TransactionCommitError(sqlx::Error),
    transaction
        .commit()
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(TransactionCommitError(e))))?;
    // ##############################################################

    // SendEmailError(reqwest::Error),
    send_confirmation_email(&email_client, new_subscriber, &base_url.0, &sub_token)
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(SendEmailError(e))))?;
...
pub enum SubscribeError {
    ...
    #[error(transparent)]
    //todo catch all unexpected error
